/*
Client to test and call server functionalities
Help will be provided on wrong command or empty new line
Available commands: starts with tilde (~)
...
func main() {
	now := time.Now()
	sec := now.Unix()
	c := &Client{"U" + strconv.Itoa(int(sec)), "", "localhost:1234", nil}
	args := os.Args
	if len(args) > 4 {
